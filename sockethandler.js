...
exports.connection = function(socket){
    ...
    socket.on('disconnect', function(){
        ...
        for (var sock in sockets) {
          if (sockets.hasOwnProperty(sock)) {
              ...
              compModel.findCompanyByObjID(obj.doc.companyID).then(function(compObj){
                ...
                if (compObj.found){
                  if (!sockets[obj.doc.companyID]){
                    ...
                    sockets[obj.doc.companyID] = [{username:obj.doc.username, socket:socket, lastCheck:lastCheck}]; // Add this socket to the list of sockets
                  } else {
                    console.log("ADDING TO ARR");
                    sockets[obj.doc.companyID].push({username:obj.doc.username, socket:socket, lastCheck:lastCheck})
                  }
                }
                ...
                eventModel.getCompanyEvents(obj.doc.companyID, timestamp).then(function(eventObj) {
                  if (obj.found){
                    for (var sock in sockets[obj.doc.companyID]){
                      sockets[obj.doc.companyID][sock].socket.emit('new', {docs:eventObj.docs, timestamp:lastCheck});
                    }
                  }
                });
