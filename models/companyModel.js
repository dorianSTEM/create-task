...
exports.createCompany = function(name, passphrase, description){
  return new Promise(function(resolve){
    db.insertOne({name:name, passphrase:passphrase, description:description, timestamp:new Date().getTime()}, function(err, doc) { // timestamp describes when company data was last updated
        if (!err) {
            resolve(true);
        } else {
            resolve(false);
